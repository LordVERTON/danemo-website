...
5. Chaque règlement est enregistré depuis la commande concernée. La fiche calcule le solde de chaque commande à partir de ses propres règlements, sans imputation par ancienneté.
6. L’action **Facture PDF** de la fiche client crée une facture brouillon si la commande n’en possède pas encore, puis génère le PDF. À chaque règlement, la facture associée évolue automatiquement : elle passe à `paid` lorsqu’elle est soldée et retrouve `sent` si elle ne l’est plus. Si la facture existe déjà, le PDF est régénéré avec le même numéro.
7. L’action **Générer la facture** de la fiche crée un PDF récapitulatif de toutes les commandes du client. Chaque commande devient une ligne avec ses propres montants réglé et solde ; ce document récapitulatif n’insère pas de nouvelle facture en base.
...
flowchart TD
  ...
  F --> G[Ajouter règlement à la commande]
...
- Le règlement est strictement positif, associé à une commande du client, en EUR, daté au format `YYYY-MM-DD`, avec l'un des modes `bank_transfer`, `cash`, `card`, `mobile`, `other`.
...
- Le calcul de TVA et le numéro de facture sont des triggers SQL. Un trigger synchronise également le statut et la date de règlement d’une facture liée à la commande lors de chaque création, modification ou suppression de règlement. La facture récapitulative est un export PDF opérateur et ne crée pas de ligne `invoices` supplémentaire.
...
3. Ajouter un règlement valide à une commande du client, puis essayer une commande d’un autre client, un montant nul, une date invalide et un mode inconnu : contrôler 201 puis 404/400.
...
- `supabase/migrations/0001_initial_schema.sql`, `supabase/migrations/20260902090000_add_customer_payments.sql`, `supabase/migrations/20260921000100_assign_payments_to_orders.sql`
