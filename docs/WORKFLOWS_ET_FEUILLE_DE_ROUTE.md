...
- [ ] Vérifier qu'aucun secret, clé de service, mot de passe de démonstration ou fichier `.env` n'est exposé dans Git, les journaux ou le navigateur. *(En cours le 06/09/2026 : la connexion n'accepte plus d'identifiants codés en dur ni de pseudo-session écrite par le navigateur ; l’authentification passe uniquement par NextAuth/Supabase. Il reste à retirer les identifiants de développement des scripts de seed et à vérifier l’historique Git distant.)*
...
- [ ] **Modales, clavier et actions de formulaire — P0.** Normaliser toutes les modales : largeur adaptée, hauteur maximale de 90 vh, contenu défilable, fermeture explicite et bouton principal atteignable avec le clavier ouvert. Critère de recette : création d'un client, commande et règlement possible sur iPhone et Android sans perdre la saisie. *(En cours le 06/09/2026 : composants de modale normalisés, actions de commande stabilisées et modale de suivi réordonnée pour placer l’ajout d’événement avant le QR code et l’historique.)*
