...
import { isAllowedOrderStatusTransition } from '@/lib/order-status'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const updatedOrder = await ordersApi.update(order.id, { status: nextStatus }, { notificationLocation: location })
    const event = await trackingApi.addEvent({
      order_id: order.id,
      status: nextStatus,
      location: location || null,
      description: getDisplayTrackingDescription(nextStatus, description),
    ...
    await recordBusinessAudit(request, {
      ...
      entityType: 'order',
      entityId: order.id,
      changedFields: ['tracking_event', 'status'],
    })
