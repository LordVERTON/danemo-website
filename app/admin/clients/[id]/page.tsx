...
export default function ClientDetailsPage() {
  ...
  async function addPayment(event: React.FormEvent) {
    ...
    setError("")
    try {
      const response = await fetch(`/api/customers/${customerId}/payments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payment, order_id: paymentOrder.id }) })
      const result = await response.json()
    ...
    try {
      ...
      const invoiceData: InvoiceData = {
        ...
        paymentSummary: {
          ...
          payments: customer.payments.filter((entry) => entry.order_id === order.id).map((entry) => ({
    ...
    <AdminLayout title="Fiche client">
      <div className="space-y-6">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <Card><CardHeader><CardTitle className="text-base">Règlements</CardTitle><CardDescription>{summary.paymentStatus === "paid" ? "Soldé" : summary.paymentStatus === "partial" ? "Partiellement réglé" : "À régler"}</CardDescription></CardHeader><CardContent><p className="text-2xl font-bold">{euro.format(summary.remainingAmount)}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-orange-600" style={{ width: `${summary.progressPercent}%` }} /></div><p className="mt-2 text-xs text-muted-foreground">{euro.format(summary.paidAmount)} réglés sur {euro.format(summary.totalAmount)}</p><p className="mt-4 text-xs text-muted-foreground">Ajoutez un règlement depuis la commande concernée.</p></CardContent></Card>
        </div>

        <Card><CardHeader className="flex-row items-center justify-between gap-3"><div><CardTitle>Commandes</CardTitle><CardDescription>{customer.orders.length} commande(s) associée(s)</CardDescription></div><Button className="hidden sm:inline-flex" onClick={() => openOrder()}><PackagePlus className="mr-2 size-4" />Nouvelle commande</Button></CardHeader><CardContent><div className="space-y-3">{customer.orders.length ? customer.orders.map((item) => {
          const orderPayment = summary.orderProgress[item.id]
          return <article key={item.id} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-medium">{item.order_number}</p><p className="mt-1 text-sm text-muted-foreground">{item.origin} → {item.destination} · {item.service_type}</p>{item.container_code && <p className="mt-1 text-sm text-muted-foreground">Conteneur associé : {item.container_code}</p>}<p className="mt-2 text-sm text-muted-foreground">Règlement : {euro.format(orderPayment?.paidAmount ?? 0)} réglés · solde {euro.format(orderPayment?.remainingAmount ?? Math.max(Number(item.value) || 0, 0))}</p></div><Badge variant="outline" className="shrink-0">{item.status}</Badge></div><div className="mt-4 flex flex-wrap items-center gap-2"><span className="mr-auto font-medium">{euro.format(Number(item.value || 0))}</span><Button type="button" variant="outline" onClick={() => { setPaymentOrder(item); setPayment({ amount: "", paid_at: new Date().toISOString().slice(0, 10), payment_method: "bank_transfer", reference: "", notes: "" }) }}><CreditCard className="mr-2 size-4" />Règlement</Button><Button type="button" variant="outline" onClick={() => openOrder(item)}><Pencil className="mr-2 size-4" />Modifier</Button><DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="outline" size="icon" aria-label={`Plus d’actions pour ${item.order_number}`}><MoreHorizontal className="size-5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="min-w-52"><DropdownMenuLabel>Documents</DropdownMenuLabel><DropdownMenuItem disabled={saving} onSelect={() => downloadInvoice(item)}><FileText className="mr-2 size-4" />Facture PDF</DropdownMenuItem><DropdownMenuItem disabled={saving} onSelect={() => printQrLabel(item)}><QrCode className="mr-2 size-4" />Étiquette QR</DropdownMenuItem>{item.qr_code && <><DropdownMenuSeparator /><DropdownMenuItem asChild><Link href={`/admin/qr?code=${encodeURIComponent(item.qr_code)}`}><QrCode className="mr-2 size-4" />Ouvrir le scanner</Link></DropdownMenuItem></>}<DropdownMenuSeparator /><DropdownMenuLabel>Actions</DropdownMenuLabel><DropdownMenuItem className="text-red-600 focus:text-red-600" disabled={deletingOrder} onSelect={() => { setDeleteOrderError(""); setOrderPendingDeletion(item) }}><Trash2 className="mr-2 size-4" />Supprimer la commande</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div></article>
        }) : <p className="py-6 text-center text-sm text-muted-foreground">Aucune commande pour ce client.</p>}</div></CardContent></Card>

        <Card><CardHeader><CardTitle>Historique des règlements</CardTitle></CardHeader><CardContent><div className="space-y-2">{customer.payments.length ? customer.payments.map((item) => <div key={item.id} className="flex items-center justify-between rounded-lg border p-3 text-sm"><div><p className="font-medium">{euro.format(Number(item.amount))}</p><p className="text-muted-foreground">{customer.orders.find((order) => order.id === item.order_id)?.order_number || "Commande non affectée"} · {item.paid_at} · {item.payment_method || "—"}{item.reference ? ` · ${item.reference}` : ""}</p></div><Badge variant="secondary">Enregistré</Badge></div>) : <p className="py-4 text-sm text-muted-foreground">Aucun règlement enregistré.</p>}</div></CardContent></Card>
