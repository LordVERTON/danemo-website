        ...
        <DialogContent className="overflow-x-hidden sm:max-w-xl">
          ...
          <form onSubmit={addOrder} className="min-w-0 space-y-5">
            <div className="grid min-w-0 gap-4 sm:grid-cols-2">
              ...
              <div><Label>Montant (€)</Label><Input required type="number" min="0" step="0.01" inputMode="decimal" value={order.value} onChange={(event) => setOrder({ ...order, value: event.target.value })} /></div>
            </div>
            <div><Label>Description du colis</Label><Textarea required value={order.description} onChange={(event) => setOrder({ ...order, description: event.target.value })} placeholder="Contenu, nombre de colis ou consignes utiles" /></div>
            <fieldset className="space-y-4 rounded-lg border p-4"><legend className="px-1 text-sm font-semibold">Destinataire</legend><div className="grid gap-4 sm:grid-cols-2"><div><Label>Nom</Label><Input required autoComplete="name" value={order.recipient_name} onChange={(event) => setOrder({ ...order, recipient_name: event.target.value })} /></div><div><Label>Téléphone (facultatif)</Label><Input type="tel" inputMode="tel" autoComplete="tel" placeholder="+237 6 12 34 56 78" value={order.recipient_phone} onChange={(event) => setOrder({ ...order, recipient_phone: event.target.value })} /><p className="mt-1 text-xs text-muted-foreground">Avec indicatif pays.</p></div><div className="sm:col-span-2"><Label>Email</Label><Input type="email" autoComplete="email" value={order.recipient_email} onChange={(event) => setOrder({ ...order, recipient_email: event.target.value })} /></div><div className="sm:col-span-2"><Label>Adresse</Label><Input required autoComplete="street-address" value={order.recipient_address} onChange={(event) => setOrder({ ...order, recipient_address: event.target.value })} /></div><div><Label>Ville</Label><Input required autoComplete="address-level2" value={order.recipient_city} onChange={(event) => setOrder({ ...order, recipient_city: event.target.value })} /></div><div><Label>Code postal (facultatif)</Label><Input autoComplete="postal-code" value={order.recipient_postal_code} onChange={(event) => setOrder({ ...order, recipient_postal_code: event.target.value })} /></div><div className="sm:col-span-2"><Label>Pays</Label><Input required autoComplete="country-name" value={order.recipient_country} onChange={(event) => setOrder({ ...order, recipient_country: event.target.value })} /></div></div></fieldset>
