...
function QrAdminContent() {
  ...
  return <AdminLayout title="Scan QR"><div className="mx-auto grid max-w-4xl gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle className="flex items-center gap-2"><QrCode className="size-5 text-orange-600" />Scanner une commande</CardTitle><CardDescription>Le statut et l’événement de suivi sont enregistrés après votre confirmation.</CardDescription></CardHeader><CardContent><form onSubmit={submit} className="space-y-4"><div><Label>Code QR</Label><div className="flex gap-2"><Input required value={code} onChange={(event) => setCode(event.target.value)} placeholder="ORD-…" /><Button type="button" variant="outline" onClick={cameraOpen ? closeCamera : openCamera} aria-label={cameraOpen ? "Fermer la caméra" : "Ouvrir la caméra"}><Camera className="size-4" /></Button></div></div><div><Label>Nouveau statut</Label><Select value={status} onValueChange={setStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="pending">En attente</SelectItem><SelectItem value="confirmed">Confirmée</SelectItem><SelectItem value="in_progress">En cours</SelectItem><SelectItem value="completed">Terminée</SelectItem><SelectItem value="cancelled">Annulée</SelectItem></SelectContent></Select></div><div><Label>Localisation</Label><Input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Bruxelles, port, agence…" /></div><div><Label>Note</Label><Textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Information ajoutée au suivi" /></div><Button className="w-full" disabled={saving}>{saving ? <Loader2 className="mr-2 size-4 animate-spin" /> : <ScanLine className="mr-2 size-4" />}Valider le scan</Button></form></CardContent></Card><Card><CardHeader><CardTitle>Caméra et résultat</CardTitle><CardDescription>La lecture caméra utilise les capacités natives du navigateur.</CardDescription></CardHeader><CardContent className="space-y-4">{cameraOpen && <video ref={videoRef} className="aspect-video w-full rounded-lg bg-slate-950 object-cover" muted playsInline />}{error && <p className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>}{result && <div className="rounded-lg border border-green-200 bg-green-50 p-4"><Badge>Scan enregistré</Badge><p className="mt-3 font-medium">{result.item.order_number}</p><p className="text-sm text-muted-foreground">Statut : {result.item.status}</p></div>}{!cameraOpen && !result && <div className="flex min-h-48 flex-col items-center justify-center rounded-lg border border-dashed text-center text-muted-foreground"><QrCode className="mb-3 size-8" /><p>Saisissez un code de commande ou ouvrez la caméra.</p></div>}</CardContent></Card></div></AdminLayout>
}
