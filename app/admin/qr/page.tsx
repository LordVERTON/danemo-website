      ...
      <div className="mx-auto grid max-w-4xl gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Le statut et l’événement de suivi sont enregistrés après votre confirmation.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={submit} className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <Input required value={code} onChange={(event) => setCode(event.target.value)} placeholder="ORD-…" />
              ...
              <div><Label>Note</Label><Textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Information ajoutée au suivi" /></div>
              <Button className="w-full" disabled={saving}>{saving ? <Loader2 className="mr-2 size-4 animate-spin" /> : <ScanLine className="mr-2 size-4" />}Valider le scan</Button>
            </form>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>Caméra et résultat</CardTitle><CardDescription>La lecture caméra utilise les capacités natives du navigateur.</CardDescription></CardHeader>
          <CardContent className="space-y-4">
            ...
            {result && <div className="rounded-lg border border-green-200 bg-green-50 p-4"><Badge>Scan enregistré</Badge><p className="mt-3 font-medium">{result.item.order_number}</p><p className="text-sm text-muted-foreground">Statut : {result.item.status}</p></div>}
            {!cameraOpen && !result && <div className="flex min-h-48 flex-col items-center justify-center rounded-lg border border-dashed text-center text-muted-foreground"><QrCode className="mb-3 size-8" /><p>Saisissez un code de commande ou ouvrez la caméra.</p></div>}
          </CardContent>
        </Card>
      </div>
    </AdminLayout>
